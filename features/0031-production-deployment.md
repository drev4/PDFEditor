...
The provider and the final app/API hostnames remain external decisions. Packaging can and should be portable while those are chosen.
...
8. Synchronise operations, API reference, roadmap and backlog with what is actually built. D1 remains open until a real target and hostnames exist.

## Out of scope

- Choosing or provisioning a cloud provider, domains, TLS certificates, PostgreSQL, Redis or object storage.
...
The portable package now has a pruned backend runtime image, a Prisma migration image, a separately built Nginx SPA image, a four-role production Compose model, a non-secret environment template and an operator runbook. The deployment target itself remains deliberately open: no provider, hostname, TLS certificate or managed dependency was invented in code.
