...
**Found in review, after merge, and fixed on `fix/0034-scrub-event-extra`:** `beforeSend` deleted `request`, `user` and `breadcrumbs` and stopped there. When `captureException` is handed a value that is **not an `Error`**, Sentry's *core* serialises that value's own properties into `event.extra.__serialized__` — core, not an integration, so `defaultIntegrations: false` does not govern it. Both `unhandledRejection` handlers pass the raw rejection reason straight through, and a rejection reason is often a plain object, so the backstop had a hole in exactly the path the worker depends on. Nothing in the code triggered it, but "no reachable trigger today" is not what a backstop is for. `extra` and `contexts` are now deleted too, and `backend/tests/error-tracking-scrub.spec.ts` holds the whole list — it was run against the unfixed code and two of its four cases failed.
