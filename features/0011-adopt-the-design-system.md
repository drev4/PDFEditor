...
**Verification.** All four suites green: frontend 30 specs / 250 tests, backend 12 / 115, integration 7 / 72, E2E 41 / 41. `vue-tsc` and backend `tsc --noEmit` clean.

**Getting the E2E suite to run at all took two environment fixes, neither caused by this change**, and both are now filed:

- 25 tests failed on a `429`. `playwright.config.ts` sets `reuseExistingServer: !process.env.CI`, and a backend already running on `:3000` from an earlier `npm run dev` was adopted *without* the `RATE_LIMIT_*: 100000` overrides the config passes to a server it starts itself. Registration hit the real limiter, and every test that registers or logs in timed out on `waitForURL(/\/dashboard/)` — which looks exactly like an application bug.
- With that server stopped, the same tests failed on `Internal server error`. The local `vuepdf` database had never been migrated past the baseline: no `organizations`, `memberships` or `refresh_tokens`, so registration 500ed. `prisma migrate resolve --applied 0_baseline` then `migrate deploy` fixed it — 143 users, 1 form, 5 responses and 6 answers all intact afterwards, with 143 personal organizations backfilled. The 429 had been masking this the whole time.

**One test changed for a real reason.** `form-management.spec.ts` asserted the dashboard empty state matched `/welcome/i`. The canvas has no welcome hero — the empty state is a dropzone — so the assertion now looks for the heading and the upload affordance instead of a greeting. Two others changed as the spec predicted (`pdf-workflow.spec.ts`, `PageThumbnails.spec.ts`) and one it did not predict: `example.spec.ts` asserted `toHaveTitle(/vuepdf/)` **case-sensitively**.

**Not verified: the PDF round-trip by hand** — that a field placed in the editor lands in the same place in the embedded AcroForm. Nothing automated covers it (P3 row: *PDF round-trip test*), and the argument that it is unaffected is a code argument rather than a test: no `:style` coordinate binding and no `DEFAULT_SCALE` was touched, which the diff shows. It is still worth one look in the running app before this merges.
