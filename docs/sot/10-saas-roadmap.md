...
The [build order](#build-order) at the end is the exception, and it is different in kind: it tracks **real state**. Steps 0 to 9 are closed; step 10 (public API, API keys, webhooks) is next, so the `[NOT IMPLEMENTED]` tag on this title applies to the design sections above it, not to that table.
...
**Step 10 is next, and what it now depends on is worth stating.** A public API with API keys and webhooks is the first surface where a caller drives the product without a browser, and two things step 9 built are load-bearing for it: outbound webhooks need a queue with retries, which now exists and needs only a second job type — and the shared rate-limit store, which a public API makes mandatory rather than advisable, now has its Redis. Neither is built for it yet; both are unblocked. What step 10 does **not** get for free is the observability it will need, which is still a backlog row rather than a step.
