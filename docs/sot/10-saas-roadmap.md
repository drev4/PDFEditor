...
The [build order](#build-order) at the end is the exception, and it is different in kind: it tracks **real state**. Steps 0 to 9 are closed and step 10 is half closed — API keys and the read-only API are in, webhooks are not — so the `[NOT IMPLEMENTED]` tag on this title applies to the design sections above it, not to that table.
...
| 10 | **Public API + API keys + webhooks** — **half done** | Only possible once step 1 is done. **API keys and a read-only `/api/v1` shipped in [`features/0019`](../../features/0019-api-keys-and-read-only-public-api.md)**; outbound webhooks are still open and are the other half. What the first half decided, and what the second must not undo: an API key authenticates an **organization and no user**, so `/api/v1` is its own router scoping on `organizationId` rather than a flag on the routers that resolve a `Membership` — and `/api/v1` is a **contract** while everything else under `/api/*` stays internal and free to change ([06-api-reference](./06-api-reference.md)). Two things it deliberately left out: **write endpoints**, because what a script may destroy needs its own thinking, and **the SPA's API keys tab**, so a customer cannot mint a key from the product yet. Webhooks now need only a second job type on the queue [`features/0017`](../../features/0017-job-queue-for-pdf-embedding.md) brought |
...
**Step 10 is half closed.** [`features/0019`](../../features/0019-api-keys-and-read-only-public-api.md) shipped API keys and a read-only `/api/v1`, which used both things step 9 built: the shared rate-limit store became load-bearing the moment a limit was *published* (and the API's limiter is the first that counts per key rather than per address), and the queue is what outbound webhooks — the remaining half — will need for retries and a delivery log.

What step 10 still does **not** get for free is the observability it needs. A customer's integration failing at 3am is answered today by reading stdout, and that is a backlog row rather than a step.
