...
| 9 | **Object storage + job queue** — **half done** | Required to run more than one replica; pull earlier if PDF timeouts appear. Also brings the Redis that the shared rate-limit store needs — see the inversion below. **Object storage shipped in [`features/0016`](../../features/0016-object-storage-for-uploaded-pdfs.md)**; the job queue is still open and is next. They were split rather than built together because they share the theme and nothing else — different files, different tests, separately revertible — and because the ordering matters: the queue's whole payload is PDF extraction and embedding, and a worker in another process cannot read a file on the API container's local disk. Storage had to come first or the queue would have been built against storage that was about to be replaced |
| 10 | **Public API + API keys + webhooks** | Only possible once step 1 is done |

Steps 0 through 3 are correctness and safety, not features. They come first because everything after them assumes the product does not lose data, does not fall over when pointed at, and can be verified. **They are all closed**, and so are steps 4 through 8. Step 9 is half closed — object storage is done, the job queue is next.
