...
|---|---|---|---|
| D1 | **Provision the chosen deployment target** — Railway + Cloudflare R2 and both environment definitions are documented in [`deploy/railway/README.md`](../../deploy/railway/README.md) | Operability | Provider credentials and DNS access |
...
**D1. A deployment target, and the packaging to reach it.** The target is now Railway for the SPA/API/worker plus managed PostgreSQL and Redis, with Cloudflare retaining DNS/TLS and private R2 storage. [`deploy/railway/README.md`](../../deploy/railway/README.md) defines two isolated projects: `docaiflow-development` at `app.dev.docaiflow.com` / `api.dev.docaiflow.com`, and `docaiflow-production` at `app.docaiflow.com` / `api.docaiflow.com`. Neither exists until its credentials, managed dependencies and DNS records are provisioned. Three processes need somewhere to run, not one: the API, the **worker** (`node dist/worker.js`, the same build with a different entrypoint) and the built SPA. **`REDIS_URL` set with no worker running fails silently** — no request errors, the queue just fills and every form's PDF quietly stops matching its fields; the API readiness check therefore requires a registered worker.

**Packaging is done in [`features/0031`](../../features/0031-production-deployment.md).** The backend image has runtime and one-shot migration targets, the same runtime runs API and worker, the SPA has its own Nginx image, and `/health/ready` refuses traffic when PostgreSQL is unavailable or Redis has no registered worker. D1 stays open only for actual provisioning and verification: portable containers and a provider definition are not a live environment.
