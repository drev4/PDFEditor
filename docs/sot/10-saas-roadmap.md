...
The [build order](#build-order) at the end is the exception, and it is different in kind: it tracks **real state**. Steps 0 to 5 are closed; step 6 (adopting the design system) is next, so the `[NOT IMPLEMENTED]` tag on this title applies to the design sections above it, not to that table.
...
| 6 | **Adopt the design system** across the product | A full design exists and none of it is built ([05-frontend-patterns §8](./05-frontend-patterns.md)). It is here rather than later for one reason: the canvas already contains the **Plan & usage** and **Plan limit reached** screens that step 7 has to build. Adopting the system first means building them once. This is an ordering of cost, not of possibility — step 7 is not blocked by it, it is only made cheaper |
...
Steps 0 through 3 are correctness and safety, not features. They come first because everything after them assumes the product does not lose data, does not fall over when pointed at, and can be verified. **They are all closed**, and so are steps 4 and 5. Next is adopting the design system, then `Plan` + entitlements.

### Parallel track — the landing page

**The landing is not in the chain, and putting it there would be a lie.** The chain means *each step unblocks the next*; the landing blocks nothing and nothing blocks it. It is designed (desktop and phone artboards in the same canvas), it needs a technology decision nobody has taken, and it can be built at any point — including before step 6, if reaching customers matters more than the product looking finished when they arrive.
