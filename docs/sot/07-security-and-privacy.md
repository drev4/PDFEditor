...
OUTBOUND — the only requests this product makes to an address a CUSTOMER chose
...
`services/webhook-egress.ts` is the only module allowed to make that request, and it does four things, each of which is load-bearing: **https only** (a signature proves who sent a payload and does nothing to keep respondent answers off the wire); **resolve and check every address** (checking the hostname string checks nothing); **connect to the address that was checked**, by pinning it into `https.request`'s `lookup` (two independent resolutions are a DNS-rebinding hole); and **no redirects** (a `302` to the metadata service undoes the other three in one hop). Plus a total timeout and a response-size cap, because a customer's endpoint is not a trusted peer. The URL is re-checked on **every delivery**, not only when it was saved — verified by hand: an endpoint whose host later resolved to `127.0.0.1` was refused at delivery time and disabled, with the reason in its delivery log. There are now **three** call sites of `assertDeliverableUrl`, and the rule behind them is one sentence: it runs at every point where delivery becomes possible. Configuration is the first, delivery the second, and re-enabling a disabled endpoint the third ([`features/0022`](../../features/0022-webhooks-screen.md)) — a hostname that was public when it was saved can point inside this network by the time somebody presses *Re-enable*, and that path would otherwise revive an endpoint nobody had re-checked.

**The webhook signing secret is the first live secret stored in a row rather than in the environment.** It cannot be hashed like an API key, because it has to be used to sign, so it is encrypted with AES-256-GCM under `WEBHOOK_SIGNING_KEY`. The honest scope, so nobody claims more later: it does **nothing** against a compromised application process, which holds the key by definition, and **everything** against the far more common incident — a leaked backup, a snapshot handed to a contractor, a read-only injection — where the database is obtained without the environment. A deployment without that key cannot configure webhooks at all (`503`), rather than silently storing plaintext.

**The log holds no respondent data either** ([`features/0025`](../../features/0025-structured-logging.md)). Nothing writes a request body to the log — not at `debug`, not behind a flag — because answers arrive keyed by field id, so their paths are *data* and no redaction list can enumerate them. The `redact` configuration (`authorization`, `cookie`, `password`, `token`, `secret`) is a backstop against a line that forgets, not the mechanism: logging everything and then removing the known-bad paths fails **open**, and this codebase logs only what it names. Same rule, one layer up, as the webhook delivery log that stores no payloads.

**The organization-wide response listing carries no respondent data, and that was a decision** ([`features/0024`](../../features/0024-organization-responses.md)). `GET /api/organizations/responses` returns which form, when and how many answers — no answer values, no `ipAddress`, no `userAgent` — while `GET /api/forms/:id/responses` returns whole `Response` rows and its screen renders an IP column. The difference is not inconsistency: one is a customer opening one form they own, the other is a browsing surface over everything the organization has ever collected, left open on a desk. Widening the second to everything the first shows would enlarge **S7** below without adding anything the screen needs, so the body is built field by field and `backend/tests/integration/organization-responses.spec.ts` asserts the three absences by name.

**The active organization is a choice, not a credential** ([`features/0023`](../../features/0023-active-organization.md)). `User.activeOrganizationId` says which organization a person is acting in when they belong to more than one. It is deliberately **not** in the access token, for the reason already established for membership itself: a 15-minute token cannot be revoked, so a claim baked into one outlives the thing it claims. And it is deliberately not a header the client supplies, because a call site that forgets to send it acts in whatever the fallback is — silently, which is the shape of the defect this replaced. What makes it safe is that `requireMembership` only ever uses it to **select among memberships that already exist**: a value naming an organization the caller does not belong to selects nothing and falls back, so the column can never widen what anybody reaches, and removing a member takes effect on their very next request. Asserted in `backend/tests/integration/tenancy.spec.ts`.
...
| ~~S3~~ | ~~**Author-supplied regex executed on a public endpoint.**~~ **Resolved on the server** ([`features/0004`](../../features/0004-safe-author-supplied-regex.md)) and **now in the browser too** ([`features/0035`](../../features/0035-bounded-pattern-validation-in-the-browser.md)). Patterns are compiled by RE2 on the API, which cannot backtrack, so execution is linear in input length: the case that took 155 s on a native `RegExp` now takes 0.05 ms. Patterns are also validated when stored, so an invalid one is rejected with a `400` instead of turning every later submission into a 500. The respondent's browser kept the backtracking engine until 0035 — see below. | ~~High~~ | `backend/src/services/pattern-validator.ts`, `frontend/src/services/pattern-check.ts` |
...
| S7 | **Partly closed** ([`features/0032`](../../features/0032-respondent-notice-and-ip-collection.md)). `ipAddress` and `userAgent` are now stored only when the form's author turns `Form.collectsRespondentMetadata` on, which is **off by default**, and the public form carries a notice saying where the answers go and — only when true — that the address is recorded. **What is still open:** there is no retention limit (it needs a scheduler this codebase does not have), the addresses collected before this change are still stored, and a respondent still has no way to reach or erase their own answers. Whatever the form author asks for in the fields themselves — which in the target market means health, financial or employment data — is unchanged and is the author's basis, not ours. | **Medium** (legal: high) | `routes/responses.ts` · `routes/forms.ts` |
| S8 | ~~**No account deletion and no data export.**~~ — **closed.** Erasure in [`features/0029`](../../features/0029-account-deletion-and-real-erasure.md) (`DELETE /api/account`) and portability in [`features/0030`](../../features/0030-account-data-export.md) (`GET /api/organizations/export`). Note what is still true: a **respondent** has no way to reach their own answers, which belongs with the respondent notice in S7 and is filed. | **Medium** (legal: high) | `routes/account.ts` · `services/organization-export.ts` |
...
| ~~S12~~ | ~~**`Form.pdfUrl` had no owner, so a share link was a capability to another tenant's document.**~~ **Resolved** ([`features/0039`](../../features/0039-uploads-belong-to-an-organization.md)). `createFormSchema` and `updateFormSchema` accepted any `z.string()` and `canonicalPdfUrl` validated shape alone — and **the filename is not a secret**: `signPdfUrl` puts it in the URL path and `GET /api/forms/public/:shareId` hands that to every respondent, because their browser has to fetch the document. So anyone ever sent a share link could create their own form on that key and be issued fresh signed URLs indefinitely, surviving the original being unpublished or deleted — S1's expiry defeated without forging anything. It ran backwards too: `collectOrphanDocuments` keeps a key while any surviving form references it, so the stranger's row **pinned the victim's document alive against the victim's own deletion request**, quietly falsifying S8's erasure. A `pdfUrl` must now name an `Upload` row owned by the acting organization. Note the rejection is one `400` for "not yours" and "does not exist" alike — two answers would be an enumeration oracle. | ~~High~~ | `backend/src/services/uploads.ts` |
...
Correctly handled today, and worth not regressing: uploaded PDFs are reachable only through a signed, expiring URL, the URL persisted in `Form.pdfUrl` is always the unsigned canonical one, and it may only name a document the acting organization uploaded; ownership failures return 404 rather than 403; the public form endpoint strips `userId`; the 500 handler never leaks internal messages; user selects are explicit so `passwordHash` cannot escape; CORS is pinned to a single configured origin; the service refuses to start without `JWT_SECRET`.
...
## The same pattern, two engines, and why the browser still needs a bound

[`features/0004`](../../features/0004-safe-author-supplied-regex.md) closed S3 on the API and left the other half open: `frontend/src/composables/useFormValidation.ts` still ran the author's pattern through `new RegExp` in the **respondent's** browser, on every keystroke of a public form. [`features/0035`](../../features/0035-bounded-pattern-validation-in-the-browser.md) closed it.

**The severity was higher than it had been filed at, and the reason is that the two engines disagree.** RE2 and JavaScript were compared directly against this code:

| Pattern | RE2 (what the server stores) | Native `RegExp` (what the respondent ran) |
|---|---|---|
| `^(a+)+$` | **accepted**, linear, 0.05 ms | **accepted**, catastrophic |
| `(?P<n>a)` | **accepted** | `SyntaxError` |
| `(?=.*\d).{8,}` | rejected — `invalid perl operator: (?=` | accepted |

The first row is the finding. `^(a+)+$` passes `checkPattern` because RE2 has no reason to object, so it is stored — and then hangs the tab of everyone who fills in that form. Measured while building 0035: still running after **25 seconds** on 41 characters. **The server-side fix did not make this safe; it moved where the unsafe engine ran.**

**The remedy is different from the server's, and deliberately so.** `pattern-validator.ts` argues that a synchronous `test()` cannot be bounded by a timeout, because the event loop is what it blocks — true, and why the API needs an engine that cannot backtrack. A browser has a second thread it may **kill**: the pattern runs in a Worker and `terminate()` stops it mid-`test()`. Verified in real Chromium: ordinary patterns answer in 0.3 ms, `^(a+)+$` is killed at 50 ms, and the page stays responsive.

**RE2 was deliberately not shipped to the browser.** WebAssembly needs `'wasm-unsafe-eval'` in `script-src`, weakening the one directive [`features/0007`](../../features/0007-security-headers-and-csp.md) measured and kept tight — on every page, including the public form — and it would add hundreds of kilobytes to what a respondent downloads. The Worker needs no CSP change at all: `worker-src 'self' blob:` is already there for pdf.js.
...
**Deleting a form now deletes its PDF** ([`features/0029`](../../features/0029-account-deletion-and-real-erasure.md), `services/pdf-gc.ts`), which closes the half of this that mattered most on object storage — a document the customer deleted was otherwise data retained, and billed for, indefinitely. **And the document a form may point at is now one the organization uploaded** ([`features/0039`](../../features/0039-uploads-belong-to-an-organization.md), S12), which is what makes that deletion reliable rather than defeasible by a stranger's row.

Unchanged and still open: **revocation is all-or-nothing** — withdrawing access to one PDF still means rotating `JWT_SECRET` or deleting the file. In [`docs/BACKLOG.md`](../BACKLOG.md).
...
6. **A privacy layer** (S7): a retention policy per form, an IP-collection toggle and a respondent notice on the public form. ~~Account deletion~~ and ~~per-account export~~ are done ([`features/0029`](../../features/0029-account-deletion-and-real-erasure.md), [`features/0030`](../../features/0030-account-data-export.md)); S8 is closed.
...
|---|---|---|---|---|
| Email, name, password hash | Account holder | `users` | Contract | Until the account holder deletes it — `DELETE /api/account` erases the row immediately, with no grace period ([`features/0029`](../../features/0029-account-deletion-and-real-erasure.md)) |
| Uploaded PDF, **and the filename the customer gave it** | Account holder | Local disk or an object store, served only through a signed expiring URL; the ownership record is the `uploads` table, which stores `key`, `original_name`, `size` and who uploaded it. **`original_name` is customer-chosen free text** and routinely carries a name (`John_Smith_passport.pdf`) — it is display only, never used to build a path, and goes with the document | Contract | Until no form references it any more, at which point `services/pdf-gc.ts` removes the object and its `uploads` row ([`features/0029`](../../features/0029-account-deletion-and-real-erasure.md), [`0039`](../../features/0039-uploads-belong-to-an-organization.md)). That is the last form being **deleted** or, since [`features/0046`](../../features/0046-editor-save-collects-the-replaced-document.md), being **repointed** at another document — so an edit saved in the editor erases the version it replaced rather than retaining it indefinitely. Documents orphaned before those existed are still stored, and finding them is filed |
...
| Everything above, **again, as a single export** | Account holder **and respondent** | `GET /api/organizations/export`, streamed and never stored | Contract, and the data subject's right to portability | Not stored by this route — but it is the widest way out of the building this product has: one request produces every form, every answer, every respondent IP address and user agent the organization holds. It is bounded by three live checks rather than anything remembered: the caller's membership and role are re-read per request, the scope is the **active** organization only, and it is rate limited per user (5/hour by default). Note the honest limit of the role check — a member can already assemble the same data one form at a time through the per-form CSV ([`features/0030`](../../features/0030-account-data-export.md)) |
...
**Sentry is a subprocessor whenever a DSN is configured**, and it is the second one on a list this product still does not formally have. Two things bound it, and both are enforced in code rather than promised here. What is sent is an **allowlist**, because the reason a denylist cannot work for the log applies identically: answer values arrive keyed by field id, so their paths are the customer's data and no fixed set of names covers them. The backstop deletes `request`, `user`, `breadcrumbs`, `extra` and `contexts` — the last two added after review found that Sentry's core, which `defaultIntegrations: false` does not govern, serialises a non-`Error` capture into `event.extra`. And **the respondent surface reports nothing at all** — the three routes marked `meta: { public: true }` are excluded at the call site and again in `beforeSend`, so a person filling in a public form is never the subject of an event. Session Replay is not enabled anywhere. Details and the accepted cost are in [08-operations](./08-operations.md#error-tracking).
...
That row used to read *"Legitimate interest (anti-abuse) — but not documented, not disclosed, and not currently used for anti-abuse"*, and this document called it the weakest position in the table: data collected for a stated purpose the system does not implement is hard to defend, so either use it for abuse prevention and say so, or stop collecting it. [`features/0032`](../../features/0032-respondent-notice-and-ip-collection.md) took the second option. Collection is off unless the author asks for it, and when they do the respondent is told. What is **not** closed is retention: nothing here deletes anything on a schedule, and saying otherwise would be a claim rather than a control.
...
5. **User-supplied code-like input** — regex, templates, formulas, file names — is untrusted input with a resource cost, not just a value to validate. Compile it through one audited helper, never at the call site: `backend/src/services/pattern-validator.ts` is the model on the server and `frontend/src/services/pattern-worker.ts` is the only place the SPA may construct a `RegExp`. Remember that a synchronous evaluator **cannot** be bounded by a timeout on the same thread — which is why the server needs an engine that cannot backtrack, and why the browser needs a thread it is allowed to kill.
