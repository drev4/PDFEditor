...
| S8 | ~~**No account deletion**~~ — **closed** by [`features/0029`](../../features/0029-account-deletion-and-real-erasure.md): `DELETE /api/account` erases the account, every organization the caller is alone in, and the stored documents of the forms that go with them. **No data export**, the other half, is still open — so today the only way out of the product is to lose everything, which is why erasure shipping first is a sequencing note rather than a completed answer to portability. A respondent still has no way to reach their own answers. | **Medium** (legal: high) | `routes/account.ts` · export: no endpoint exists |
...
6. **A privacy layer** (S7, S8): a retention policy per form, an IP-collection toggle, a respondent notice on the public form, ~~account deletion~~ (done, [`features/0029`](../../features/0029-account-deletion-and-real-erasure.md)), and per-account export.
