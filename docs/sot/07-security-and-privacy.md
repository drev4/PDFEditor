...
```
Anonymous internet
  ├── POST /api/responses              writes to the database, no auth, no throttle
  ├── GET  /api/forms/public/:shareId  reads a form, no auth, mutates viewCount
  └── GET  /uploads/pdfs/<file>        reads any uploaded PDF, no auth, no expiry
...
The three anonymous entry points are the whole external attack surface, and none of them is rate limited.
...
| S2 | **No rate limiting anywhere.** `POST /api/auth/login` permits unlimited credential stuffing against 6-character passwords. `POST /api/responses` permits unlimited garbage submissions into any published form, with no CAPTCHA or throttle, which is both a data-integrity and a cost problem. | **High** | all routes |
...
1. **Rate limiting** (S2) on `/api/auth/login` and `/api/responses`. Hours of work, removes the two easiest attacks.
...
2. **Every new public endpoint ships with rate limiting**, from the first one that gets it.
