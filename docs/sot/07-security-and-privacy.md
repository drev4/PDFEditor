...
| Uploaded PDF, **and the filename the customer gave it** | Account holder | Local disk or an object store, served only through a signed expiring URL; the ownership record is the `uploads` table, which stores `key`, `original_name`, `size` and who uploaded it. **`original_name` is customer-chosen free text** and routinely carries a name (`John_Smith_passport.pdf`) — it is display only, never used to build a path, and goes with the document | Contract | Until the last form referencing it is deleted, at which point `services/pdf-gc.ts` removes the object and its `uploads` row ([`features/0029`](../../features/0029-account-deletion-and-real-erasure.md), [`0039`](../../features/0039-uploads-belong-to-an-organization.md)). Documents orphaned before that existed are still stored, and finding them is filed |
