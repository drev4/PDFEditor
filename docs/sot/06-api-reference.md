...
| PUT | `/forms/:id` | Bearer + ownership | Partial `{title?, description?, status?, pdfUrl?, settings?}`. **`402`** when `status: 'published'` would exceed the plan's published-form limit. **`400`** when `pdfUrl` names no upload of the form's organization. **Side effect:** a `pdfUrl` naming a *different* key hands the replaced one to `services/pdf-gc.ts` after the write ([`features/0046`](../../features/0046-editor-save-collects-the-replaced-document.md)) — the object goes only if no surviving form still references it, and a storage failure is logged, never returned |
...
**An upload is still not consumed by being used — but a key a form is repointed *off* is now collected.** Two forms may share one document and the row stays usable for both, which is why the collector asks whether any surviving form references the key rather than which form used to. What changed in [`features/0046`](../../features/0046-editor-save-collects-the-replaced-document.md) is that `PUT /api/forms/:id` asks it: repoint the only form using a key, and the object and its `Upload` row go. Pointing something back at that key afterwards gets the same `400` as any other unusable value. An upload no form ever referenced is untouched by this — nothing triggers a collection for a key no `PUT` moved away from.
...
No endpoint returning an organization's name, no pagination beyond `/forms/:id/responses`, no bulk response deletion, and no way to delete a single response. **The rest of this paragraph was stale and is corrected here rather than deleted**, because it named things that have since shipped and a reader trusting it would have rebuilt them: the public API and API keys are [`features/0019`](../../features/0019-api-keys-and-read-only-public-api.md) and [`0021`](../../features/0021-api-keys-screen.md), outgoing webhooks are [`features/0020`](../../features/0020-outbound-webhooks.md), and the two GDPR obligations are closed — account deletion is `DELETE /api/account` ([`features/0029`](../../features/0029-account-deletion-and-real-erasure.md)) and the whole-organization export is `GET /api/organizations/export` ([`features/0030`](../../features/0030-account-data-export.md)), both documented above. Billing covers all three plans as of [`features/0015`](../../features/0015-team-plan-and-purchased-seats.md). What is deliberately absent there: no endpoint changes an existing subscription's plan or its seat quantity — both are Stripe's portal, and this application never sends a quantity.
