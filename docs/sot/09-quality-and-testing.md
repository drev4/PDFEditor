...
|---|---|---|---|
| Frontend unit / component | 41 specs, 343 tests | Vitest, `@testing-library/vue`, `@pinia/testing`, jsdom (`frontend/vitest.config.ts`) | Beside the code, `frontend/src/**/*.spec.ts` |
| Backend route (mocked Prisma) | 18 specs, 231 tests | Vitest, `supertest`, `vitest-mock-extended` | `backend/tests/*.spec.ts` |
| **Backend database-backed** | 23 specs, 193 tests | Vitest, `supertest`, **real PostgreSQL** (`backend/vitest.integration.config.ts`) | `backend/tests/integration/*.spec.ts` |
| End to end | 8 specs, 51 tests | Playwright, Chromium | `e2e/*.spec.ts`, helpers in `e2e/helpers.ts` |
