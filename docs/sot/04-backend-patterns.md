...
The same rule is why every counted plan limit runs inside the transaction that writes: a count is a read whose result decides a write. See §10.
...
`services/uploads.ts` is the third, and it owns **who may point a form at a key** ([`features/0039`](../../features/0039-uploads-belong-to-an-organization.md)). The division with `pdf-url.ts` is the reason it is a separate module rather than a function added to it: that module is deliberately pure and knows nothing about the database, while this one asks a single question of it — does an `Upload` with this key belong to this organization. **`assertUploadBelongsTo` is the only ownership check on a `pdfUrl`, and `pdfFilenameFrom` is still the only parser**; the check calls the parser and never re-implements it. Note the deliberate flatness of its answer: "not yours" and "does not exist" are one `400` with one message, because two answers would let a caller enumerate real filenames without reading one.

`services/pdf-storage.ts` is the fourth, and it owns the **bytes** ([`features/0016`](../../features/0016-object-storage-for-uploaded-pdfs.md)). The division with `pdf-url.ts` is worth keeping straight: that module owns what a URL may contain and how a filename is safely got out of one; this module owns where the bytes live and how they move. **Nothing outside it may join an `uploads` path or open a PDF by name** — six call sites used to do exactly that (four routes, `app.ts`, and a maintenance script no test covers), and leaving any one of them behind in a move to object storage gives a deployment where uploads work and one read silently 404s depending on which replica answered.
...
**No limit is read-then-written, and the signature is where you read that.** A function in `entitlements.ts` that *refuses* something takes a `Prisma.TransactionClient` as its first parameter, runs inside the same transaction as the write it guards, and nothing may catch what it throws before the transaction boundary. A function that only *reports* — `getEntitlements`, `isOverResponseLimit`, `mustShowBranding`, `assertHasApiAccess` — passes `prisma` and does not, because it writes nothing and there is no window to close.

There are two mechanisms, because there are two kinds of limit:

- **The response meter claims a counter.** `assertResponseWithinLimit(tx, organizationId)` upserts the counter with an `increment` and compares the value it gets back, inside the same transaction as the `Response`. The upsert takes the row lock, so a second concurrent submission blocks and then reads `limit + 1` and throws — and the throw rolls back the increment and the response together. Read-compare-increment would let two submissions both pass at `limit - 1`, and a compensating decrement is a thing to get wrong.
- **The counted limits lock the organization row.** `assertCanPublishForm(tx, organizationId, formId?)` and `assertCanInvite(tx, organizationId)` count rows that exist right now, so there is no counter to claim — they call `lockOrganization(tx, organizationId)`, a `SELECT id FROM organizations WHERE id = $1 FOR UPDATE` through tagged `$queryRaw`, as the **first statement of the transaction**, before any count ([`features/0027`](../../features/0027-atomic-plan-limits.md)).

Four things about the row lock, because each of them is a way to write code that looks right and is not:

- **A transaction alone is not enough.** Under Postgres's default `READ COMMITTED` two concurrent transactions both run the count before either commits its write, neither sees the other's uncommitted row, and both pass. A transaction makes the pair atomic; it does not serialise them.
- **The order is the mechanism.** Taking the lock after the count restores the race exactly. That is why it is a named function called on the first line rather than an option passed somewhere.
- **No counter column was added.** `UsageCounter` exists because responses are *events accepted in a period*, which a `count(*)` cannot express. Published forms and seats are current state, and a stored count of them would be a cache of a number the database already knows, free to drift with no meaning behind it.
- **`SERIALIZABLE` was the other correct answer and costs more.** It moves the failure into a retry loop: a serialization failure is Prisma `P2034`, which is neither a `402` nor something a client can act on, so every call site would need retry logic — and a retry re-runs a handler that has already validated, checked a role and read a form. The row lock blocks and then proceeds, and adds no new failure mode.

Note what this is that `services/organization-lock.ts` is not. That file refuses a database lock and stays refusing one: its subject is `POST /api/billing/checkout`, where on the first checkout there is **no subscription row to lock**, and where covering the work means holding a transaction open across a call to Stripe. Neither objection survives here — the organization row always exists, and these handlers write locally and return. The in-process lock covers one Node process; this one is in Postgres, which every replica shares.
