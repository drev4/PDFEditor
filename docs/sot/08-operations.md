...
**Schedulable since [`features/0042`](../../features/0042-scheduled-backups.md).** `npm run backup` is one command that does both halves in order, each run into its own timestamped directory, and it **refuses to start without `BACKUP_DIR`** — no default, because `./backups` inside a discarded container is a job that succeeds nightly and keeps nothing. It runs from the `backup` target of `Dockerfile.backend`, which exists because **no other image has `pg_dump`** and Debian's is 15 against a 16 server.

The scope of the beta, the repeatable production check and the go/no-go gate are in [`docs/runbooks/beta-launch.md`](../runbooks/beta-launch.md). Its check list is built from the failures that produce no error anywhere — a missing worker, a compiled-in DSN that was never rebuilt, `TRUST_PROXY_HOPS`, a backup that copies zero documents — which is why it is a written list rather than clicking around.

**What is still open.** The schedule itself and the alert are the deployment's, and the repository can now only hand it an honest exit code — **nothing alerts** when a backup fails, so a scheduled job that does not notify on a non-zero exit turns a broken backup into a discovery made at restore time. There is also **no off-site copy**: a volume beside the database survives a container, not the loss of the account. Recovery time was measured on 2026-09-02 against a development dataset and the procedure passed, but **the production figure is unknown until there is a production database to measure**, and the runbook says to re-run the drill within a week of the first deploy.
