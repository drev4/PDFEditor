...
| Item | Why |
|---|---|
| Enforce `Membership.role` | `owner \| admin \| member` are stored and never read, so every member of an organization can do everything including delete it. Harmless while every organization has exactly one member; it stops being harmless the moment invitations ship — so it is specced **with** them, in [`features/0010`](../features/0010-member-invitations-and-role-enforcement.md) | [07-security](./sot/07-security-and-privacy.md) · [`features/0009`](../features/0009-organizations-own-resources.md) |
...
| Member invitations | The first feature that makes B2B real rather than a table with one row. `Organization` and `Membership` exist; what is missing is a way to add a second member. **Next**, per the [build order](./sot/10-saas-roadmap.md#build-order). Specced as [`features/0010`](../features/0010-member-invitations-and-role-enforcement.md), **together with role enforcement** — neither is safe to ship without the other |
